  ...
  return class ICSical extends ICSmodule {
    ...
    async populateICalEvents(eventsArr = []) {
      function genIcal(eventData) {
        ...
        ics.createEvent(eventData, (error, value) => {
          ...
          fs.writeFile(
            path.join(icsFilePath, `./${eventData.productId}_event.ics`),
            value,
            (err) => {
      ...
      for (let inx = 0; inx < eventsArr.length; inx++) {
        ...
        try {
          ...
          if (this.debug) log({ error, populateICalEvents: true });
    ...
    CreateEventData(absenceWithMember) {
      ...
      return new (function (absenceMember) {
        ...
        this.created = () => {
          const c = moment(createdAt || null).isValid()
            ? moment(createdAt).utc().toArray()
            : null;
        ...
        this.start = () => {
          const c = moment(startDate || null).isValid()
            ? moment(startDate).toArray()
            : null;
          if (c) c.splice(6); // max size is 6
          return c ? icsDateAdjustment(c, 1) : null;
        };
        ...
        this.lastModified = () => {
          const c = moment(confirmedAt || null).isValid()
            ? moment(confirmedAt).utc().toArray()
            : null;
          if (c) c.splice(6); // max size is 6
          return c ? icsDateAdjustment(c, 1) : null;
        };
        ...
        this.end = () => {
          ...
          const c = moment(dateStr).isValid()
            ? moment(endDate).toArray()
            : null;
          if (c) c.splice(6); // max size is 6
          return c ? icsDateAdjustment(c, 1) : null;
        };
        ...
        this.duration = () => {
          const d = (absence_days || []).length
            ? { days: head(absence_days).toString() }
            : null;
          if (d && Number((d || {}).days) < 1) {
            if (self.debug)
              log("[CreateEventData] absence_days must be gte >0");
            return null;
          } else return d;
        };
        ...
        this.description =
          (admitterNote || "").length > (memberNote || "").length
            ? admitterNote
            : memberNote || "";

        // TENTATIVE, CONFIRMED, CANCELLED
        this.status =
          this.start && !rejectedAt
            ? "CONFIRMED"
            : rejectedAt
            ? "CANCELLED"
            : "TENTATIVE";
        ...
        this.organizer = () => {
          ...
          return pickBy(
            { name: name, email: "admin@domainname.com" },
            identity
          );
        };
        ...
        this.event = () => {
          ...
          const lastTest =
            Object.keys(validEnv).filter((a) => {
              return requiredFields.filter((b) => {
                // at least 1 should match
                if (isArray(b))
                  return b.filter((c) => a.indexOf(c) !== -1).length > 0;
                // all should match
          ...
          if (lastTest) return validEnv;
            ...
            if (self.debug)
              log(
                "[CreateValidEventData] failed [requiredFields,...] validation"
              );
            return null;
          }
        };
    ...
    typeSetMessage(memberName, absenceType = "") {
      ...
      switch (absenceType) {
        ...
        default:
          if (this.debug)
            log(`[typeSetMessage] wrong absenceType: ${absenceType}`);
      }
      return message;
    }
  };
