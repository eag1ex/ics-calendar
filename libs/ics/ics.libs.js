...
module.exports = (ICSmodule) => {
  ...
  const { log, head, someKeyMatch, copy } = require("x-utils-es/umd");
  ...
  class ICSlibs extends ICSmodule {
    ...
    QueryChecked(query) {
      return new (function () {
        ...
        this.ok = () => {
          return (
            this.query.filter((item) => item["match"] === true).length ===
    ...
    queryFilter(data = [], query = {}, limitedSearch = [], dbName = "") {
      if (
        !limitedSearch ||
        !(limitedSearch || []).length ||
        limitedSearch.indexOf("ALL_ITEMS") !== -1
      ) {
      ...
      if (!limit()) {
        if (this.debug) log("[queryFilter] no match for limitedSearch");
        this.d = [];
        return this;
      }
          ...
          if (isStart || isEnd) {
            const withStartDate = () => {
              ...
              if (query["startDate"] && el["startDate"])
                return (
                  date(query["startDate"]).getTime() <=
                    date(el["startDate"]).getTime() && isStart
                );
              return false;
            };
            ...
            const withEndDate = () => {
              ...
              if (query["endDate"] && el["endDate"])
                return (
                  date(el["endDate"]).getTime() <=
                    date(query["endDate"]).getTime() && isEnd
                );
              return false;
            };
    ...
    assignMember(includeMember = null) {
      ...
      const arrAsync = copy(absencesList).map(async (item) => {
        ...
        if (member) {
          ...
          if (this.debug)
            log(
              `[absences] no member for userId:${item.userId} found on members.db `
            );
        }
    ...
    assignAbsences(includeAbsence = null) {
      ...
      const arrAsync = copy(memberList).map(async (item) => {
        ...
        if (absences.length) item["absences"] = absences;
          ...
          if (this.debug)
            log(
              `[absences] no absences for userId:${item.userId} found on absences.db `
            );
        }
        return item;
      });
