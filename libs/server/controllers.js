...
module.exports = function (expressApp) {
  const { isNumber, log, objectSize, copy } = require("x-utils-es/umd");
  ...
  return class ServerController {
    ...
    calendar(req, res) {
      if (this.serverError)
        return res
          .status(500)
          .json({ message: "ICS databse error", code: 500 });

      const validTypes = this.ics.availableAbsenceTypes.filter(
        (z) => z === req.params.type
      ).length;
      if (!validTypes)
        return res.status(200).json({ error: true, ...messageCodes[108] });
      const type = req.params.type;
      const userId = Number(req.params.userId);
      if (!isNumber(userId) || userId < 0)
        return res
          .status(404)
          .json({ error: "wrong userId provided", response: {}, code: 200 });

      return this.ics
        .generateICS(type, userId, "members")
        .then((z) =>
          res
            .status(200)
            .json({
              success: true,
              response: z,
              ...this.ics.statusHandler.$get(),
            })
        )
        .catch((error) =>
          res.status(400).json({ error, ...messageCodes[601] })
        );
    }
    ...
    database(req, res) {
      if (this.serverError)
        return res
          .status(500)
          .json({ message: "ICS database error", code: 500 });
      ...
      if (collection === "absences") {
        ...
        return (async () => {
          const response = copy(
            await this.ics.absences(query, includeMember)
          ).map((item) => {
            if (item["type"] && item["member"])
              item["type"] = this.ics.typeSetMessage(
                item.member.name,
                item.type
              );
            return item;
          ...
          res.status(404).json({ error, ...messageCodes[600] })
        );
      }
      if (collection === "members") {
        ...
        return (async () => {
          const response = copy(
            await this.ics.members(query, [], showAbsence)
          ).map((item) => {
            if (showAbsence) {
              // update type message
              item["absences"].forEach((element) => {
                if (element.type)
                  element.type = this.ics.typeSetMessage(
                    item.name,
                    element.type
                  );
              });
          ...
          res.status(404).json({ error, ...messageCodes[601] })
        );
